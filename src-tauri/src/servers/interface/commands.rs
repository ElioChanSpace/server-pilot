...
fn read_between_markers(output: &str) -> Result<&str, String> {
    let start = output
        .find(METRICS_OUTPUT_START)
        .ok_or_else(|| "Metrics output start marker not found".to_string())?;
    let end = output
        .find(METRICS_OUTPUT_END)
        .ok_or_else(|| "Metrics output end marker not found".to_string())?;

    if end <= start {
        return Err("Metrics output markers are invalid".to_string());
    }

    Ok(&output[start + METRICS_OUTPUT_START.len()..end])
}

fn parse_metrics_output(output: &str) -> Result<ServerMetricsSnapshot, String> {
    let metrics_block = read_between_markers(output)?;
...
fn run_ssh_metrics_command(
    ...
    loop {
        if Instant::now() > deadline {
            let _ = child.kill();
            return Err("Timed out while collecting server metrics".to_string());
        }
            ...
            Err(mpsc::RecvTimeoutError::Timeout) => {
                if let Some(status) = child.try_wait().map_err(|err| err.to_string())? {
                    if output.trim().is_empty() {
                        return Err(format!("Failed to collect server metrics: {}", status));
                    }
                }
            }
...
pub async fn fetch_server_metrics(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        let output = run_ssh_metrics_command(
            &username,
            &host,
            port,
            password.as_deref(),
